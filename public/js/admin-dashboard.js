...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function tryFetchHosts() {
    const candidates = ['/api/admin/hosts', '/api/users?role=host', '/api/users'];
  ...
  function renderHosts(list = hosts) {
    if (!list || list.length === 0) {
      hostsTableContainer.innerHTML =
        '<p class="text-muted">No se encontraron anfitriones.</p>';
      return;
    }
    ...
    list.forEach((h) => {
      ...
      const roles =
        h.roles && typeof h.roles === 'string'
          ? h.roles
          : Array.isArray(h.roles)
          ? h.roles.join(', ')
          : JSON.stringify(h.roles || '[]');
      const activo =
        h.activo === 0 || h.activo === false ? 'No' : 'Sí';
      ...
      html += `<tr data-id="${id}">
        ...
        <td>
          ...
          <button class="btn btn-sm btn-warning btn-toggle-block">${
            activo === 'Sí' ? 'Bloquear' : 'Desbloquear'
          }</button>
        </td>
      </tr>`;
    });
    ...
    document
      .querySelectorAll('.btn-view-props')
      .forEach((b) => b.addEventListener('click', onViewProps));
    document
      .querySelectorAll('.btn-view-rooms')
      .forEach((b) => b.addEventListener('click', onViewRooms));
    document
      .querySelectorAll('.btn-toggle-block')
      .forEach((b) => b.addEventListener('click', onToggleBlock));
  }
  ...
  async function onViewProps(e) {
    ...
    document.getElementById(
      'modalPropsTitle'
    ).innerText = `Propiedades del anfitrión ${id}`;
