...
document.addEventListener('DOMContentLoaded', () => {
  ...
  filterInput.addEventListener('input', () => renderHosts(filterHosts(filterInput.value)));
  ...
  function filterHosts(q) {
    ...
    return hosts.filter(h => {
      const name = (h.nombre_completo || h.nombre || '').toString().toLowerCase();
      const email = (h.correo_electronico || h.email || '').toString().toLowerCase();
  ...
  async function loadHosts() {
    ...
    try {
      ...
      if (path === '/api/users') {
        list = data.filter(u => {
          const roles = u.roles || [];
          if (Array.isArray(roles)) return roles.some(r => /host|anfitri|propietar/i.test(String(r)));
          if (typeof roles === 'string') return /host|anfitri|propietar/i.test(roles);
          return false;
        });
      }
      hosts = list;
      ...
      console.error(err);
      hostsTableContainer.innerHTML = '<p class="text-danger">No se pudo cargar la lista de anfitriones.</p>';
    }
  }
  ...
  async function onViewProps(e) {
    ...
    try {
      ...
      // acciones
      document.querySelectorAll('.btn-delete-prop').forEach(b => b.addEventListener('click', async ev => {
      ...
      document.querySelectorAll('.btn-edit-prop').forEach(b => b.addEventListener('click', ev => {
        const pid = ev.currentTarget.dataset.id;
        window.location.href = `/admin/edit-property.html?id=${pid}`;
      }));

      document.querySelectorAll('.btn-view-rooms-prop').forEach(b => b.addEventListener('click', ev => {
        const pid = ev.currentTarget.dataset.id;
        const name = ev.currentTarget.dataset.name || pid;
        // reusar modal rooms viewer
        // trigger rooms modal for the property
        fetch(`${API}/api/properties/${pid}/habitaciones`, { credentials: 'same-origin' })
          .then(r => { if (!r.ok) throw new Error(); return r.json(); })
          .then(rooms => {
            let html = `<h6>Habitaciones de ${name}</h6>`;
            if (!rooms.length) html += '<p class="text-muted">No hay habitaciones.</p>';
            else {
              ...
              rooms.forEach(room => {
                html += `<div class="list-group-item">
                    ...
                    <div class="mt-1">
                      <button data-id="${room.id_habitacion}" class="btn btn-sm btn-danger btn-del-room">Eliminar</button>
                    </div>
                  </div>`;
              });
              html += '</div>';
            ...
            document.querySelectorAll('.btn-del-room').forEach(btn => btn.addEventListener('click', async ev2 => {
              ...
              if (!rr.ok) {
                ...
                return alert(err.error || 'Error al eliminar habitación');
              }
              alert('Habitación eliminada');
              // refresh property rooms view
              ev.currentTarget.click();
            }));
          })
          .catch(err => alert('Error al cargar habitaciones'));
      }));
  ...
  // === ver habitaciones de un anfitrión (todas sus habitaciones) ===
  async function onViewRooms(e) {
    const id = getRowIdFromBtn(e.currentTarget);
    if (!id) return alert('ID no encontrado');
    document.getElementById('modalRoomsTitle').innerText = `Habitaciones del anfitrión ${id}`;
    document.getElementById('modalRoomsBody').innerHTML = 'Cargando...';
    modalRooms.show();

    try {
      const r1 = await fetch(`${API}/api/properties/host/${id}`, { credentials: 'same-origin' });
      if (!r1.ok) throw new Error('Error al obtener propiedades');
      const props = await r1.json();
      if (!props || props.length === 0) {
        document.getElementById('modalRoomsBody').innerHTML = '<p class="text-muted">No hay propiedades.</p>';
        return;
      }
      let allRooms = [];
      for (const p of props) {
        try {
          const r2 = await fetch(`${API}/api/properties/${p.id_propiedad}/habitaciones`, { credentials: 'same-origin' });
          if (!r2.ok) continue;
          const rooms = await r2.json();
          rooms.forEach(room => { room._propiedad = p.nombre_propiedad; room._id_propiedad = p.id_propiedad; });
          allRooms = allRooms.concat(rooms);
        } catch (err) { continue; }
      }
      if (!allRooms.length) {
        document.getElementById('modalRoomsBody').innerHTML = '<p class="text-muted">No hay habitaciones registradas.</p>';
        return;
      }
      ...
      allRooms.forEach(room => {
        html += `<div class="list-group-item">
          ...
          <div class="mt-1 small">Precio: $${room.precio_por_noche || 0} - Capacidad: ${room.capacidad_maxima || '-'}</div>
          <div class="mt-2">
            <button class="btn btn-sm btn-danger btn-del-room-global" data-id="${room.id_habitacion}">Eliminar</button>
            <button class="btn btn-sm btn-secondary btn-edit-room" data-id="${room.id_habitacion}" data-prop="${room._id_propiedad}">Editar</button>
          </div>
        </div>`;
      });
      html += '</div>';
      ...
      document.querySelectorAll('.btn-del-room-global').forEach(b => b.addEventListener('click', async ev => {
      ...
      document.querySelectorAll('.btn-edit-room').forEach(b => b.addEventListener('click', ev => {
        const rid = ev.currentTarget.dataset.id;
      ...
      console.error(err);
      document.getElementById('modalRoomsBody').innerHTML = '<p class="text-danger">Error al cargar habitaciones.</p>';
    }
  }
  ...
  async function onToggleBlock(e) {
    ...
    try {
      const host = hosts.find(h => String(h.id_usuario || h.id || '') === String(id));
      const isActive = !(host && (host.activo === 0 || host.activo === false));
